...
| --- | --- | --- | --- | --- |
| 2026-07-04T10:15:07+0300 | resilience-batch | Plan 39 batch/resident resilience. DONE: exit-code semantics (F6) in `run_resident_batch.py`. NEXT: resident OOM-retry + ML→heuristic fallback + per-run degradations/result_quality in `resident_pipeline.py`. NOT touching `apps/ore_pipeline_web.py`/`component_analysis.py`/`tests/test_ore_pipeline_web.py` (held by ore-grain-reporting-impl). | `scripts/run_resident_batch.py`, `src/ore_classifier/resident_pipeline.py`, `docs/plans/39_pipeline-resilience-and-recovery.md` | active |
...
| 2026-07-04T10:09:51+0300 | ore-grain-reporting-impl | Implementing Plan 42 grain-reporting API/UI fields, focused tests, docs handoff, and commit. | `src/ore_classifier/component_analysis.py`, `apps/ore_pipeline_web.py`, `apps/static/ore_pipeline_ui.html`, `tests/test_ore_pipeline_web.py`, `docs/ui/v2/specs/ore-pipeline-grain-reporting-v0.1.md`, `docs/ui/v2/plans/42_ore-pipeline-grain-reporting.md`, shared `ChangeLog.md`/`docs/session-sync.md` as narrow handoff updates | active |
