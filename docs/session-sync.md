...
- `docs/plans/28_ore-pipeline-ui.md`: implemented local browser/canvas UI plan for upload, preprocessing, immutable runs, visualization, edit-and-recalculate, exports, and history.
...
- Upload supports click-to-open and drag/drop for PNG, JPEG, TIFF, and RAW-extension files. Unsupported dropped/selected extensions show an inline localized warning before upload. Large uploads show byte-transfer progress followed by preview-preparation progress while the server decodes and scales display pyramids; manual preprocessing preview updates use the same progress row. RAW decoding uses optional `rawpy`; regular formats use Pillow.
...
- Display layers use generated preview pyramids for original, preprocessed, sulfide overlay, and final class overlays so large images are not pushed into the browser at full resolution. Scaled/large-image preprocessing records a tile manifest that the `show tiling` viewer checkbox can draw over original, preprocessed, sulfide, final, and side-by-side views.
- Preprocessing controls cover illumination normalization, denoising, contrast correction, and panorama scaling. All four controls default to enabled, the user's chosen preset persists in local storage across reloads/upload clears, and each run records the selected preset plus the preprocessed analysis image.
- Result views cover primary original, preprocessed, sulfide/non-sulfide, and final segmentation layers. Layers stay disabled/greyed until their artifacts exist, and side-by-side is a separate none/preprocessed/sulfide/final comparison selector with the selected layer on the right and a draggable splitter. Final classes can hide/show background, ordinary intergrowth, fine intergrowth, and talc.
- Page routing uses slug URLs: `/` redirects to `/workspace`, `/workspace` and `/history` are direct-loadable, tab clicks update the URL, and browser back/forward restores the selected page.
...
- Metrics include total sulfide fraction, ordinary/fine intergrowth fractions, talc fraction, component count, and analyzed area. CSV and PDF report exports are generated per run.
- The History page uses a table with thumbnail, filename, date, ore classification, sulfide/non-sulfide and final-class fractions, plus `Load` and `Remove` actions. Clicking a thumbnail opens a modal preview from the run preview pyramid. `Load` restores the original upload and preprocessing preset for tuning; `Remove` deletes the selected completed/failed run artifact from history through `DELETE /api/runs/{run_id}` while leaving uploads intact.
- `Fix me` opens a brush editor with sulfide/non-sulfide and final segmentation tabs, a top toolbar for Brush/Pan, undo/redo, zoom in/out, fit view, and brush size, plus live pixel/% statistics for sulfide, non-sulfide, ordinary intergrowth, fine intergrowth, and talc. The dialog refreshes the full run payload before loading editor assets so artifact URLs are re-registered after server restarts/stale browser state; missing masks/images show a localized editor error instead of an empty canvas. `Fix and Restart` always creates a derived run with `parent_run_id`; sulfide edits recalculate final segmentation, while final edits recalculate metrics/report without replacing the parent sulfide mask.
- `tests/test_ore_pipeline_web.py` covers upload, preprocessing, full run, CSV/PDF artifacts, sulfide edit rerun, final edit recalculation, history, and required UI controls.
...
- Preferred talc review UI is now `apps/talc_review_web.py`: a local `http.server` app with generated HTML/CSS/vanilla JS canvas. It can start from the raw annotated `Области оталькования` folder or `outputs/talc_blue_line_conversion`, pairs clean originals by exact filename, auto-creates `current_talc_mask.png` on first open, edits the talc mask directly, supports base image modes for original/MS Paint/QA/current-mask/sulfide-mask backgrounds, supports a single toolbar ordered Brush/Rectangle/Polygon/SAM2/Undo/Zoom In/Zoom Out/Fit with active-tool parameters plus top-right `Save` and `Save & Next` actions, supports mouse-wheel zoom, supports brush left-draw/right-erase with a hover-radius circle, polygon, rectangle, and SAM2 canvas tools plus undo/autosave, supports persistent `System`/`Light`/`Dark` themes, has polygon click-to-add and close-by-clicking-first-point, right-click point removal, right-click draft cancel away from points, rectangle drag or two-corner drawing, editable completed polygon/rectangle regions with point/edge/corner/whole-shape drag, Delete/Backspace selected-shape removal, live filled-mask updates, save-time shape flattening, default `SAM2 box` prompting, dashed SAM2 hover/drag prompt previews, idle `SAM2 point` hover masks shown as non-destructive orange previews with explicit `Apply SAM2`, SAM2 box/polygon prompt clipping plus oversized-mask rejection, default-on protection against new sulfide overlap plus manual sulfide subtraction, and writes reviewed masks, overlay, edit patch, and summary under each sample's `reviewed/` directory.
- `tests/test_talc_review_web.py` covers annotated/original pairing, first-open current-mask creation, reviewed save artifacts, reset behavior, theme/sulfide-guard/brush-button controls, and HTTP JSON endpoints; `tests/test_sam2_region_assist.py` covers SAM2 prompt clipping and oversized-mask rejection. Local `python3 -m unittest discover -s tests -p 'test_*.py' -v` covers `65` tests.
