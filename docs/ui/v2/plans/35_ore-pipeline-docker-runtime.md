...
4. Add a root Compose file that builds the image, publishes `8080`, persists `outputs/ore_pipeline_ui`, mounts `models` read-only for optional checkpoint use, and keeps a GPU profile for gx10/ML deployments.
...
- `docker compose up --build` starts the UI on `http://<vm-host>:8080/workspace`.
