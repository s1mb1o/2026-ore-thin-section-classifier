...
class TalcReviewWebTest(unittest.TestCase):
    ...
    def test_page_includes_persistent_theme_selector_and_dark_mode_css(self) -> None:
        ...
        self.assertIn("z-index: 20", markup)
        self.assertIn("min-width: 58px", markup)
        self.assertIn("html, body { width: 100%; height: 100%; overflow: hidden; }", markup)
        self.assertIn("width: 100vw; max-width: 100vw; height: 100vh", markup)
        self.assertIn(".work-pane { min-width: 0; width: 100%; max-width: 100%;", markup)
        self.assertIn("flex: 1; min-width: 0; min-height: 0; max-width: 100%; overflow: auto", markup)
        ...
        self.assertIn(".zoom-widget-row, .zoom-widget-main { display: grid; grid-template-columns: 40px", markup)
        self.assertIn(".zoom-widget button {\n  width: 40px;\n  height: 40px;", markup)
        self.assertIn(".zoom-widget svg {\n  width: 20px;\n  height: 20px;", markup)
        ...
        self.assertIn("function zoomPercentText()", markup)
        self.assertIn("function handleZoomWidgetPointer(event)", markup)
        self.assertIn("function runZoomControl(event, action, messagePrefix)", markup)
        self.assertIn("if (state.viewPan.active) finishViewPan();", markup)
        self.assertIn("els.zoomWidget.addEventListener(eventName, handleZoomWidgetPointer", markup)
        self.assertIn("runZoomControl(event, fitToViewer, 'Fit view')", markup)
        self.assertIn("runZoomControl(event, actualSizeView, 'Actual size')", markup)
        self.assertIn("function setZoom(value, anchor = null, options = {})", markup)
        self.assertIn("const resetPanOrigin = Boolean(options.resetPanOrigin);", markup)
        self.assertIn("updatePanGutter({ preserveCanvasPosition: !resetPanOrigin });", markup)
        self.assertIn("if (resetPanOrigin) {", markup)
        self.assertIn("function viewerViewportSize()", markup)
        self.assertIn("function viewerPadding(wrap)", markup)
        self.assertIn("width: Math.max(0, Math.round(wrap.clientWidth || rect.width || 0))", markup)
        self.assertIn("height: Math.max(0, Math.round(wrap.clientHeight || rect.height || 0))", markup)
        self.assertIn("const viewport = viewerViewportSize();", markup)
        self.assertIn("x: Math.max(PAN_GUTTER_MIN_PX, viewport.width)", markup)
        self.assertIn("const extraX = Math.max(0, viewport.width - padding.left - padding.right - canvasWidth);", markup)
        self.assertIn("resetViewPanOrigin({ center: Boolean(options.centerView) });", markup)
        self.assertIn("const maxW = Math.max(240, viewport.width - 36);", markup)
        self.assertIn("const maxH = Math.max(180, viewport.height - 36);", markup)
        self.assertIn("updatePanGutter({ preserveCanvasPosition: false });", markup)
        self.assertIn("setZoom(Math.min(maxW / state.imageW, maxH / state.imageH, 1), null, { resetPanOrigin: true, centerView: true });", markup)
        self.assertIn("setZoom(1, null, { resetPanOrigin: true });", markup)
        self.assertIn("els.zoomActualWidgetBtn.addEventListener('click', (event) => runZoomControl(event, actualSizeView, 'Actual size'))", markup)
        ...
        self.assertIn("margin: var(--pan-gutter-y) var(--pan-gutter-x) var(--pan-gutter-y) var(--pan-gutter-x)", markup)
        self.assertIn("const PAN_GUTTER_MIN_PX = 240", markup)
        self.assertIn("function updatePanGutter(options = {})", markup)
        self.assertIn("function resetViewPanOrigin(options = {})", markup)
        ...
        self.assertIn("let scrollLeft = state.panGutter.x", markup)
        self.assertIn("scrollLeft = state.panGutter.x - extraX / 2", markup)
        self.assertIn("wrap.scrollLeft = Math.max(0, Math.round(scrollLeft))", markup)
