...
class OrePipelineDockerArtifactsTest(unittest.TestCase):
    def test_docker_runtime_files_keep_vm_contract(self) -> None:
        ...
        legacy_compose = (ROOT / "docker-compose.ore-pipeline-ui.yml").read_text(encoding="utf-8")
        ...
        self.assertIn("name: nornikel-ore-pipeline-ui", compose)
        self.assertIn("${ORE_UI_PUBLIC_HOST:-0.0.0.0}:${ORE_UI_PUBLIC_PORT:-8080}:8080", compose)
        self.assertIn("${ORE_UI_WORKSPACE_HOST:-./outputs/ore_pipeline_ui}:/data/ore_pipeline_ui", compose)
        self.assertIn("${ORE_UI_MODELS_HOST:-./models}:/app/models:ro", compose)
        self.assertIn("ORE_UI_TALC_BACKEND", compose)
        self.assertIn("ORE_UI_GRADE_CHECKPOINT", compose)
        self.assertIn("ore-pipeline-ui-gpu", compose)
        self.assertIn("docker/ore-pipeline-ui/Dockerfile.gx10-ml", compose)
        self.assertIn("${ORE_UI_TALC_OUTPUTS_HOST:-./outputs/talc_segformer_folds}:/app/outputs/talc_segformer_folds:ro", compose)
        self.assertIn("driver: nvidia", compose)
        self.assertIn("ipc: host", compose)
        self.assertIn("restart: unless-stopped", compose)

        self.assertIn("${ORE_UI_PUBLIC_PORT:-8080}:8080", legacy_compose)
        self.assertIn("./outputs/ore_pipeline_ui:/data/ore_pipeline_ui", legacy_compose)
        self.assertIn("./models:/app/models:ro", legacy_compose)
        self.assertIn("ORE_UI_TALC_BACKEND", legacy_compose)
        self.assertIn("ORE_UI_GRADE_CHECKPOINT", legacy_compose)
