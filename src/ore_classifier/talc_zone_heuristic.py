...
from .component_analysis import fill_holes
...
def _extract_flakes(dark: np.ndarray, matrix: np.ndarray, cfg: TalcZoneConfig) -> np.ndarray:
    ...
    for i in range(1, num):
        if stats[i, cv2.CC_STAT_AREA] <= cap:
            continue
        comp = labels == i
        closed = cv2.morphologyEx(comp.astype(np.uint8), cv2.MORPH_CLOSE, _disk(cfg.footprint_close_radius))
        footprint = fill_holes(closed) > 0
...
def detect_talc_zones(
    ...
    if ore_mask is None:
        ...
        ore_mask = np.asarray(ore_mask) > 0
        if ore_mask.shape[:2] != (h0, w0):
            raise ValueError("ore_mask dimensions must match rgb")
...
def result_to_dict(
    result: TalcZoneResult,
    config: TalcZoneConfig,
    *,
    ore_mask_source: str | None = None,
) -> dict[str, Any]:
    record = {
    ...
    if ore_mask_source:
        record["ore_mask_source"] = ore_mask_source
    return record


def make_talc_zone_overlay(
    rgb: np.ndarray,
    zone_mask: np.ndarray,
    flake_mask: np.ndarray,
    max_side: int = 1600,
) -> np.ndarray:
    """Preview overlay for heuristic talc zones and seed flakes."""
    ...
    if scale < 1.0:
        ...
        zone = np.asarray(Image.fromarray(zone.astype(np.uint8) * 255, mode="L").resize(size, Image.Resampling.NEAREST)) > 0
        flakes = np.asarray(Image.fromarray(flakes.astype(np.uint8) * 255, mode="L").resize(size, Image.Resampling.NEAREST)) > 0
    base = np.asarray(image).astype(np.float32)
    color = np.zeros_like(base)
    color[zone] = (255, 140, 0)
    alpha = (zone.astype(np.float32) * 0.40)[..., None]
    out = np.clip(base * (1.0 - alpha) + color * alpha, 0, 255)
    out[flakes] = (10, 10, 10)
    return out.astype(np.uint8)


def save_talc_zone_outputs(
    out_dir: Path,
    rgb: np.ndarray,
    result: TalcZoneResult,
    config: TalcZoneConfig,
    *,
    ...
    """Write masks, optional overlay, and a JSON record for one classifier run."""
    out_dir.mkdir(parents=True, exist_ok=True)
    zone_path = out_dir / "talc_zone_mask.png"
    flake_path = out_dir / "talc_flake_mask.png"
    overlay_path = out_dir / "overlay.jpg"
    result_path = out_dir / "talcose_result.json"

    Image.fromarray(result.zone_mask.astype(np.uint8) * 255, mode="L").save(zone_path)
    Image.fromarray(result.flake_mask.astype(np.uint8) * 255, mode="L").save(flake_path)
    paths = {
        "zone_mask": str(zone_path),
        "flake_mask": str(flake_path),
        "result_json": str(result_path),
    }
    if write_overlay:
        Image.fromarray(make_talc_zone_overlay(rgb, result.zone_mask, result.flake_mask, max_side=max_side), mode="RGB").save(
            overlay_path,
            quality=88,
        )
        paths["overlay"] = str(overlay_path)

    record = result_to_dict(result, config, ore_mask_source=ore_mask_source)
