...
- [`apps/talc_review_web.py`](apps/talc_review_web.py) — canvas-ревью масок талька; инструмент
  ...
  (`apps/talc_review_web.py:5017`) собирает r/g/b/luma/текстуру вокруг seed-пикселя,
  `similarStats()` (`:5079`) агрегирует их, а `computeSimilarTalcPreview()` (`:5220`) выводит
  допуски. Роль приложения — производство обучающих (silver) масок, **не** движок инференса.
