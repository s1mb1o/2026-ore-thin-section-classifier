...
Дефолтные чекпойнты всех трёх ML-этапов (сульфид SegFormer-B2, тальк SegFormer-B0, Grade-CNN)
закоммичены через Git LFS. На свежем клоне подтяните их:

```bash
git lfs pull
```

Если после этого какой-то чекпойнт всё равно отсутствует (например, вы удалили файл вручную
или используете один из вспомогательных вариантов не по умолчанию — другой Grade-CNN рецепт,
не-дефолтный тальк-фолд), v2 UI откатывается к эвристическому бэкенду для этого этапа. Это не
норма, а признак того, что чекпойнт не резолвится: проверьте путь в `Settings` /
`GET /api/settings` против `MODEL_CARD.md` § «Доступность чекпойнтов». Полный ML-путь по всем
трём стадиям доступен и на свежем клоне, и на развёрнутом демо.
