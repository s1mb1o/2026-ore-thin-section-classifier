...
<div class="callout" markdown="1">
**Наши операционные определения (на QA #4 организаторы подтвердили: судят по определению команды; порог 10% сохраняем).**
- **Сульфид** — светлая рудная фаза; бинарная сегментация сульфид/не-сульфид (SegFormer-B2). Всё «серое» — не сульфид (нерудная матрица / пустая порода).
- **Тип срастания** (обычное / тонкое) — по **морфологии** восстановленного сульфидного зерна: крупное, компактное, слабо замещённое нерудной фазой → **обычное** (рядовая); ажурное/фрагментированное, сильно замещённое (высокий replacement-ratio, низкие solidity/компактность) → **тонкое** (труднообогатимая). Метку сорта по срастаниям даёт Grade-CNN; покомпонентное правило — объяснение.
- **Тальк** — нерудная фаза, детектируется тальк-сегментатором (SegFormer-B0) в пределах **не-сульфидной анализируемой области** (артефакты шлифовки исключены). **Доля талька = площадь талька / площадь анализируемой области**; **≥ 10% (строго > 10%) → оталькованная**.
- **Итог (фьюз):** тальк-ветка решает оталькованную; иначе Grade-CNN — рядовая ↔ труднообогатимая.
</div>

*Идея: правило дословно из постановки задачи. Детерминизм = воспроизводимость и объяснимость. Определения выписаны явно — на QA #4 организаторы сказали, что оценивают по определению команды (экспертной доли талька у них нет).*
...
<div class="callout" markdown="1">
**Замерено (05.07):** цель ТЗ ≤ 5 мин на 10k×10k закрыта — панорама 13330×9489 (126 Мп): **99.6–186.9 с** на трёх машинах. Крупнейшая панорама **27025×21227 (574 Мп)** — **7:08 end-to-end**, пик GPU-памяти **4777 MiB (~4.7 ГБ)**.
</div>

*Идея: показать инженерную зрелость работы с гигапикселями: 5-минутная цель ТЗ закрыта замером на трёх машинах (docs/benchmarks/07), крупнейшая панорама датасета прогнана end-to-end с постоянной GPU-памятью (docs/benchmarks/08). Оговорка: пиковая системная RAM ~20 ГБ — нужна станция с 32 ГБ.*
...
| --- | --- |
| Ошибка доли талька ≤ ±3 п.п. | **измерено** (leak-free OOF, 42 silver-масок): медиана 5.53 п.п. / mean 7.77 п.п., 33% в пределах ±3 п.п. — критерий не выполняется и не сертифицируем: эталонной доли талька нет ни у нас, ни у жюри, ни у организаторов (QA#4) |
| F1 классификации типа срастаний ≥ 90% | **закрыто**: Grade-CNN `обычное↔тонкое` macro-F1 **0.930/0.939** (held-out 230, folder-GT); полный 3-классовый фьюз (тальк-ветка ⊕ Grade-CNN) macro-F1 **0.861** (leak-free 345) |
| ≤ 5 мин на 10k×10k | **закрыто**: 126 Мп-панорама за 99.6–186.9 с (3 машины); крупнейшая 574 Мп — 7:08 |
...
- **Grade-CNN (путь A)** — сквозной `efficientnet_b3`: macro-F1 **0.930/0.939** (held-out 230) —
  критерий ТЗ «тип срастаний ≥ 90%» закрыт; полный 3-классовый фьюз (тальк-ветка ⊕ Grade-CNN)
  macro-F1 **0.861** (leak-free 345);
- **honest eval-harness** (`evaluate_official_pipeline.py`): leak-free 345-сплит, устойчивость к аугментации/препроцессингу;
- **путь B** — human-in-the-loop разметка зёрен + агрегация в сорт (каркас end-to-end, дополняет Grade-CNN);
- полноценный UI: вьюер, правка, отчёты, Серии, Статус, API, Docker.

**Ближайшие шаги:**

1. ~~сквозной бенчмарк панорамы под цель ≤ 5 минут~~ — **закрыто 05.07** (126 Мп за 99.6–186.9 с; крупнейшая 574 Мп за 7:08, GPU-пик 4777 MiB, ~4.7 ГБ);
2. ~~измерение ошибки доли талька ≤ ±3 п.п. на 42 парах~~ — **измерено 05.07**: медиана 5.53 п.п., 33% в ±3 п.п. — критерий не выполняется и не сертифицируем (эталона нет ни у кого, QA#4);
3. ~~поднять image-level F1 к цели 90%~~ — **закрыто 05.07**: Grade-CNN (путь A) macro-F1 0.930/0.939; путь B (зёрна + агрегация) остаётся дополнительным интерпретируемым вердиктом;
...
<div class="callout" markdown="1">
**Слабость эвристики (пре-лейбл).** Правило «тонкое» = OR(замещение внутри, низкая solidity, низкая compactness). Массивное зерно с однородным ядром, но **сильно рваным контуром** помечается «труднообогатимым» из-за границы — при нулевом замещении. Ровно поэтому нужна человеческая разметка: grain-классификатор учится отделять «рваный край» от «тонкого срастания».
</div>

Ценность — **интерпретируемость**: «62% зёрен тонкие — вот они подсвечены». Дополняет **Grade-CNN (путь A)** — наш сквозной `efficientnet_b3` — не заменяет его.

*Идея: наш ответ, играющий на сильную сторону — сегментацию. Grade-CNN (путь A) даёт цифру; путь B даёт объяснимый геологу вердикт поверх нашей маски. Честно: полный выигрыш требует человеческих меток зёрен, каркас уже работает end-to-end.*
