...
<div class="callout" markdown="1">
**Замерено (05.07):** цель ТЗ ≤ 5 мин на 10k×10k закрыта — панорама 13330×9489 (126 Мп): **99.6–186.9 с** на трёх машинах. Крупнейшая панорама **27025×21227 (574 Мп)** — **7:08 end-to-end**, пик GPU-памяти **4.8 ГБ**.
</div>
...
1. ~~сквозной бенчмарк панорамы под цель ≤ 5 минут~~ — **закрыто 05.07** (126 Мп за 99.6–186.9 с; крупнейшая 574 Мп за 7:08, GPU-пик 4.8 ГБ);
2. измерение ошибки доли талька ≤ ±3 п.п. на 42 парах;
3. поднять image-level F1 к цели 90% — две дорожки: **Grade-CNN (путь A)** — сквозной `efficientnet_b3` (в духе nail), **путь B** — размеченные зёрна + агрегация;
...
Ценность — **интерпретируемость**: «62% зёрен тонкие — вот они подсвечены». Дополняет **Grade-CNN (путь A)** — наш сквозной `efficientnet_b3` в духе nail-rinatovich, — не заменяет его.
