...
Сначала открываем образец двести пятьдесят пять ноль три восемь два, один, десятикратное увеличение. Сейчас на экране только исходная фотография шлифа: без классов, без подсказок и без результатов модели. Это та картинка, с которой начинается экспертная проверка.
...
Из этих линий инструмент строит положительную область. Мы называем её областью-кандидатом: внутри тальк вероятен, но мы не утверждаем, что каждый пиксель внутри уже тальк. Это просто зона поиска, где эксперту и алгоритму есть смысл внимательно смотреть.
