#!/usr/bin/env python3
"""Run tiled talc segmentation and clip predictions to non-sulfide pixels."""
...
def main() -> int:
    ...
    parser.add_argument("--sulfide-mask", type=Path, default=None)
    ...
    with tempfile.TemporaryDirectory(prefix="talc_infer_", dir=str(args.out_dir)) as tmp:
    ...
    analyzed_mask = build_analyzed_mask(rgb, min_value=args.analyzed_min_value).astype(bool)
    sulfide_mask = load_optional_mask(args.sulfide_mask, (height, width))
    non_sulfide_mask = analyzed_mask & ~sulfide_mask
    talc_bool = (prob >= args.threshold) & non_sulfide_mask
    ...
    talc_path = args.out_dir / "talc_mask.png"
    confidence_path = args.out_dir / "confidence.png"
    confidence_non_sulfide_path = args.out_dir / "confidence_non_sulfide.png"
    analyzed_path = args.out_dir / "analyzed_mask.png"
    non_sulfide_path = args.out_dir / "non_sulfide_mask.png"
    ...
    sulfide_path = None
    if args.sulfide_mask:
        sulfide_path = args.out_dir / "sulfide_mask_aligned.png"
        save_gray(sulfide_path, sulfide_mask.astype(np.uint8) * 255)
    ...
    if args.save_full_overlay:
        ...
        save_overlay(image=image, mask=talc_mask, confidence=confidence_non_sulfide, path=full_overlay_path, max_side=0)
    ...
    summary = {
        ...
        "sulfide_mask": str(args.sulfide_mask) if args.sulfide_mask else None,
        "width": width,
        "height": height,
        ...
        "paths": {
            ...
            "sulfide_mask_aligned": str(sulfide_path) if sulfide_path else None,
...
def load_optional_mask(path: Path | None, target_hw: tuple[int, int]) -> np.ndarray:
    ...
    with Image.open(path) as image:
        ...
        expected_size = (target_hw[1], target_hw[0])
