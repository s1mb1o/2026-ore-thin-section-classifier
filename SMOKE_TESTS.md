...
- The Settings page Security section can set a password. After saving a password, a fresh unauthenticated browser opening `/workspace` is redirected to `/login`, `/api/settings` returns `401`, correct login returns an HttpOnly session cookie, and the Settings API exposes only `auth.password_enabled`. Checking `remove password` and saving disables the gate.
...
- `GET /api/openapi.json` returns the OpenAPI 3.1 document (`openapi` = `3.1.0`) describing every `/api/*` and `/artifacts/` route. It is served unauthenticated even when a UI password is set (so tooling can read it before login), and can be pasted into any OpenAPI viewer / imported into Swagger UI, Postman, or Insomnia. Quick check: `curl -s http://127.0.0.1:<port>/api/openapi.json | python3 -c "import sys,json;d=json.load(sys.stdin);print(d['openapi'], len(d['paths']),'paths')"`.
...
- With `Detect image decode bombs` enabled (default; Settings → Security), uploading a decompression-bomb image (small file, huge declared dimensions — e.g. a 46000×46000 PNG, ~2116 MP) is rejected with HTTP 413 before decoding and shows the localized "possible decode bomb — disable in Settings" warning; real dataset images upload normally, including the largest stitched panoramas (`dataset/Панорамы/16.jpg`, ~574 MP) which stay under the 1000 MP cap. Turning the setting off in Settings → Security lets the oversized image through the check.
...
- After image selection, `Configuration...` is enabled below `Edit Metadata...`. It opens the run configuration dialog; the `Talc clusterization` section has radius, minimum local talc percent, and opacity fields initialized from Settings defaults or the loaded run. Saving configuration affects the next `Start`/prepared run and must be persisted in the immutable run as `talc_clusterization`.
- In `final` view, both left and right segmentation legends include `talc cluster areas` with the pink swatch and a percentage. Toggling it hides/shows only the cluster-area overlay, not the underlying blue talc class. Completed runs should contain `masks/talc_cluster_mask.png`, `display.talc_cluster_overlay`, and metrics/CSV/PDF rows for talc cluster area.
...
- The result section shows text output first and the metrics table below it at full width. The table is hierarchical: analyzed area, total sulfides with ordinary/fine children, talc with talc-cluster-area child, other analyzed area, and image artefacts.
...
- Fine-intergrowth source grains that are below the dataset p20 area threshold but have ordinary-looking solidity/compactness show a `мелк` badge plus a detail/Tinder line explaining the small-grain context, and receive extra review-value weight.
...
- Direct URLs like `/tinder/<grain_uid>` open the app in `Tinder mode` focused on that grain, and the URL updates as arrow/postpone actions advance to another grain.
...
- Opening or selecting a sample updates the browser URL to `/sample/<slug>` using the current image slug, and directly opening that URL loads the same sample.
...
- The viewer also shows a separate top-right `Display layers` widget with a checked `Background` checkbox plus display-only `Blank White`, `Original blue lines`, `Talc cluster areas`, and `Sulfides` rows. Unchecking `Background` hides only the base image while selected mask/QA/blue-line/cluster/sulfide overlays remain visible; if `Blank White` is checked at the same time, the empty base is filled white. Turning on `Original blue lines` shows the raw blue annotation strokes. Turning on `Talc cluster areas` highlights locally dense non-sulfide talc regions and shows the highlighted-area percentage in the top-right widget; turning on `Sulfides` tints the sample sulfide mask as an independent overlay. The widget stays inside the visible center work/viewer area before image load, after sample load, and during resize, without overlapping the right settings panel.
- The top toolbar is ordered icon-only `Brush`, `Fill`, `Similar`, `Rectangle`, and `Polygon` with hover titles, then `SAM2`, `Undo`, and active-tool parameters; it does not show duplicate `Zoom In`, `Zoom Out`, `Fit`, or toolbar zoom percentage controls. `Save`, `Save & Next`, transparent `Next`, and icon-only `Download` stay pinned at the top right. If the toolbar wraps at narrower widths, every wrapped row remains inside the topbar and the viewer starts below it; Brush shows brush size with a `2-240 px` range, Similar shows `Strictness`, `+ seed`, `- seed`, `Apply Similar`, and `Clear Preview`, and SAM2 shows prompt mode and `Load SAM2`.
- Pressing `Download` in the top-right actions downloads `<sample>_enabled_layers.png` at the sample image resolution, composited from the current enabled background, editable class masks, display layers, comparison mode overlay, and talc-cluster overlay. The file must not include UI widgets, brush cursor, draft handles, or other transient editing chrome.
- The bottom-left viewer zoom widget is a single vertical stack: `Fit`, `Actual size`, `Zoom in`, the live zoom percentage, and `Zoom out`; it shares state with mouse-wheel zoom, `Fit` centers the fitted image in the visible viewer, and `Actual size` returns the canvas to `100%`. Repeated clicks on `Fit` or `Actual size` must not keep moving or resizing the image, and every accepted zoom-widget click updates the status line so an idempotent action does not look ignored.
...
- Selecting `Heuristic` shows only the non-neural talc-zone layer. Selecting `Current vs Heuristic` adds a comparison overlay against the current `Talc` mask. Pressing `Run non-neural classifier` in either heuristic mode calls `/api/samples/{sample_id}/talcose-heuristic`, uses the sample sulfide mask as the ore exclusion mask when available, and writes `qa/non_neural_talcose/talc_zone_mask.png`, `talc_flake_mask.png`, `overlay.jpg`, and `talcose_result.json`. The comparison overlay highlights agreement, heuristic-only in fuchsia (`#ec4899`), current-only, and sulfide-conflict pixels.
- Selecting `Neural Model` shows only the trained model talc prediction when `model_talc_mask.png` or an explicit `--talc-model-mask-dir` artifact is available; otherwise the panel offers `Run model` for the current sample. The panel exposes `ML talc probability threshold` with the current default `0.50`; changing it before `Run model` sends that probability threshold to `/api/samples/{sample_id}/neural-model`, runs `scripts/infer_talc_segmentation.py`, writes `qa/neural_talc_model/` plus sample-level `model_talc_mask.png`, and refreshes the neural layer. Selecting `Current vs Neural Model` compares that prediction against the current `Talc` mask and highlights agreement, neural-only, current-only, and sulfide-conflict pixels. Selecting `Heuristic vs Neural Model` compares the non-neural talc-zone mask against the neural mask, keeps both `Run non-neural classifier` and `Run model` available, and highlights agreement, heuristic-only, neural-only, and sulfide-conflict pixels.
...
- `Save` writes `reviewed/reviewed_positive_bag_mask.png`, `reviewed/reviewed_talc_node_mask.png`, `reviewed/reviewed_not_talc_mask.png`, compatibility union `reviewed/reviewed_talc_mask.png`, `reviewed/reviewed_ignore_mask.png`, `reviewed/reviewed_overlay.png`, `reviewed/review_patch.json`, and `reviewed/review_summary.json`.
