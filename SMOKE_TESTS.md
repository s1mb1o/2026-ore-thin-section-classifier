...
- With `Detect image decode bombs` enabled (default; Settings → Security), uploading a decompression-bomb image (small file, huge declared dimensions — e.g. a 46000×46000 PNG) is rejected with HTTP 413 before decoding and shows the localized "possible decode bomb — disable in Settings" warning; real dataset images (≤26 MP) upload normally. Turning the setting off in Settings → Security lets the oversized image through the check.
